...
$mac = $_REQUEST['mac'];
if ($stmt = $mysqli->prepare("SELECT name FROM shields WHERE mac = ?;")) {
	$stmt->bind_param("s",$mac);
        $stmt->execute();   
        $stmt->store_result();
	
        if ($stmt->num_rows == 1) {
					$stmt->bind_result($nome_scheda);
					$stmt->fetch();
				}
}
?>
		...
		<div id='settings-div'>
			...
			<fieldset>
				<table align=center cellpadding=5>
					...
					<tr><td><button onClick="location.assign('deleteShield.php?mac=<?php echo $mac ?>')">Elimina scheda</button></td></tr>

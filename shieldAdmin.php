				...
				if ($stmt = $mysqli->prepare("SELECT pin_number, name, isused, in_mode  FROM pins WHERE mac_shield = ? AND type = ? ORDER BY pin_number ASC;")) {
					...
					if($stmt->num_rows>0){
						...
						$i=0;
							...
							echo "<tr>";
							echo "<td>".$numero_pin."<td class='boldtd'>".$nome."<td> ".$usato."<td>".$input_type;
							if($str_status=="OFF"){
				...
				if ($stmt = $mysqli->prepare("SELECT pin_number, name, isused, out_mode FROM pins WHERE mac_shield = ? AND type = ? ORDER BY pin_number ASC;")) {
					...
					if($stmt->num_rows>0){
						...
						while($stmt->fetch()){
							...
							echo "<tr>";
							echo "<td>".$numero_pin."<td class='boldtd'>".$nome."<td>".$usato."<td>".$out_mode;
							if($str_status=="OFF"){
								echo "<td class='offtd'>";
							}
						...
						echo "<td><button onClick=\"location.assign('/shieldSettings.php?mac=".$mac."')\">Impostazioni Scheda</button>";
						echo "</table>";
					}
					else{
